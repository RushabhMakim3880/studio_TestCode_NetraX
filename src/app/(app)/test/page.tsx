...
const OperatorV2SplashScreen = () => {
  ...
  <div className="relative h-full w-full flex flex-col items-center justify-center p-4 font-mono overflow-hidden" style={{ backgroundColor: 'rgb(0,0,0)'}}>
    <style jsx>{`
      ...
      .glitch-text::before { left: 2px; text-shadow: -2px 0 #ff00c1; animation: glitch-before 2s infinite linear alternate-reverse; }
      .glitch-text::after { left: -2px; text-shadow: 2px 0 #00fff9; animation: glitch-after 1.5s infinite linear alternate-reverse; }
      @keyframes glitch-before { 0%, 15%, 45%, 100% { clip-path: inset(100% -1px 0 0); } 25% { clip-path: inset(25% -1px 50% 0); } 55% { clip-path: inset(60% -1px 20% 0); } 80% { clip-path: inset(80% -1px 5% 0); } }
      @keyframes glitch-after { 5%, 30%, 70% { clip-path: inset(0 -1px 100% 0); } 20% { clip-path: inset(50% -1px 25% 0); } 40% { clip-path: inset(20% -1px 60% 0); } 90% { clip-path: inset(95% -1px 0 0); } }
    ...
    <div className="w-full max-w-lg flex flex-col items-center text-center">
        ...
        <h1 className="text-2xl font-bold tracking-widest text-white mt-4 relative glitch-text" data-text="NETRA-X">
          NETRA-X
        </h1>
...
const DeadlineSplashScreen = () => {
    ...
    useEffect(() => {
        const progressInterval = setInterval(() => {
            setProgress(p => p >= 100 ? 0 : p + 100 / (animationTime * 10));
        }, 100);
    ...
    return (
        <div className="relative h-full w-full flex flex-col items-center justify-center bg-black p-4 font-mono overflow-hidden">
            <style jsx>{`
            #deadline-container { display: flex; flex-direction: column; align-items: center; justify-content: center; }
            #logo-container { width: 400px; height: 400px; margin-bottom: -120px; z-index: 10; }
            #deadline { width:581px; max-width: 100%; height:158px; position: relative; z-index: 1; }
            ...
            @keyframes move-arm { 0% { transform: rotate(0); } 5% { transform: rotate(0); } 9% { transform: rotate(40deg); } 80%, 100% { transform: rotate(0); } }
            @keyframes move-tool { 0% { transform: rotate(0); } 5% { transform: rotate(0); } 9% { transform: rotate(50deg); } 80%, 100% { transform: rotate(0); } }
